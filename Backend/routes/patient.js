...
router.post('/', (req, res) => {
    ...
    patient.save().then((patient) => {
        ...
        Doctor.findOneAndUpdate({ _id: req.body.doctor }, { $push: { patients: patient._id } });
        // Link patient to user
        User.findOneAndUpdate({ _id: req.body.user }, { $set: { profile: patient._id } }).catch((err) => {
            console.error(err);
        });
    }).catch((err) => {
...
router.get('/', (req, res) => {
    ...
    Patient.findOne(query).then((patient) => {
        if (!patient) {
...
router.patch('/', async (req, res) => {
    ...
    await Doctor.findOneAndUpdate({ _id: patient.doctor }, { $pull: { patients: patient._id } }).catch((err) => {
    ...
    await Doctor.findOneAndUpdate({ _id: req.body.doctor }, { $push: { patients: patient._id } }).catch((err) => {
